...
dnl eliminate spurious output from GCC configuration that's not pertinent
case "$CC" in
  *gcc*|*gfortran*|*g77*)
	ac_[]_AC_LANG_ABBREV[]_v_output=`echo "$ac_[]_AC_LANG_ABBREV[]_v_output" | sed '/^Configured with:/,/^Thread model:/d'`
	;;
esac
	...
	*libgcc.a | *libgcc_s.a)
	  ...
	  case $[2] in
		...
		;;
          ...
          ;;
	-Wl,-rpath)
	  # This takes an argument and can be converted to a single argument
	  case $[2] in
	    -Wl,*)
	      ac_arg="$ac_arg"`echo "$[2]" | sed -e "s/^-Wl,//g"`
	      shift; shift
	      set X $ac_arg "$[@]"
	      ;;
	    *)
...
case $host_os in
aix*)
   ac_cv_[]_AC_LANG_ABBREV[]_libs="$ac_cv_[]_AC_LANG_ABBREV[]_libs -Wl,-bbigtoc"
   ;;
esac
